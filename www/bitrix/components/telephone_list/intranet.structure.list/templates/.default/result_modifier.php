...
foreach ($arResult['USER_PROP'] as $fields) {
    if ($fields['USER_TYPE_ID'] == 'iblock_element') {
        foreach ($arResult['USERS'] as $field){
            if ($field[$fields['FIELD_NAME']] != '') {
                $staff[]=$field[$fields['FIELD_NAME']];
            }
        }
    }
}

$res = CIBlockElement::GetList(Array(), Array("ID"=> $staff), false, false, Array("ID", "NAME"));
while($ob = $res->Fetch()){
    $arResult['IBLOCK_ELEMENT_USER_FIELDS_VALUE'][$ob['ID']] = $ob['NAME'];
}
/*pre($arResult['IBLOCK_ELEMENT_USER_FIELDS_VALUE']);*/
