...
function CreateAvAlertPopup2(e, o) {
    return $(".av-alert-popup").remove(),
        $('<div class="av-alert-popup">' + e + "<div data-close-form2></div></div>").appendTo("body").on("click", "[data-close-form2]", function () {
...
$(function () {
    function e() {
        ...
        function t(e) {
            if (!$("#map_canvas").is("[markers-seted]")) {
                for (var o = [
                    // ['м. Дніпро, вул. Карла Маркса, 2<br> Телефон: <a href="tel:+38 (056) 790-01-22">+38 (056) 790-01-22</a><br><a href="tel:+38 (056) 790-73-00 ">+38 (056) 790-73-00</a>', 48.4518948, 35.0717819, 17]
    ...
    $(document).on("keydown", ".inputtext.phone", function (e) {
        ...
        if ($(this).scrollTop() > o / 2 && e() || $(this).scrollTop() > 700) {
            $(".main-wrap .main-menu-fixed").addClass("fixed");

        } else {
            $(".main-wrap .main-menu-fixed").removeClass("fixed");
            $(this).scrollTop() >= $(".about-box").offset().top / 2;
            $(".lang-drop-list-hide-part").removeClass('select');
            $("#wire-fixed-lang-drop .lang-drop-list i").removeClass('fa-angle-up').addClass('fa-angle-down');
            //menuTop.appendChild(langPanel);
                animScroll();
        }
    }),
...
$(document).ready(function () {
    ...
    // ЗАКАЗАТЬ ЗВОНОК
    $(".call-btn").parent().on("click", function () {
    ...
    $('.map-txt .contact-btn').on("click", function () {
        AvBlurScreen("on", "9999");
    ...
    $('.av-banner .av-form-button').on('click', function (e) {
        e.preventDefault();
    });
    ...
    //HIDE FORM TO BUTTON CLICK
    $('.send-btn > input').click(function(){
        $('.make-diler-form').hide();

        setTimeout(function(){
            $('.av-alert-popup').fadeOut('fast'),
            AvBlurScreen("off")
        },3000);
    });
