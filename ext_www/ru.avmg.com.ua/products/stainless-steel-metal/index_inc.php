<br>
<div class="prod-slidedown-box">
<h3 class="av-spoiler-header">Направление "Нержавеющий металл"</h3>
<p class="av-spoiler-body">АВ металл групп - компания № 1 на украинском рынке нержавеющей стали, крупнейший поставщик нержавеющего плоского проката для украинской промышленности и один из лидеров в поставках сварных нержавеющих труб и проволоки.</p>

<h3 class="av-spoiler-header">Наши поставщики</h3>
    <div class="av-spoiler-body">
        Мы тесно сотрудничаем со всеми европейскими производителями:
        <ul>
            <li>ACERINOX</li>
            <li>APERAM</li>
            <li>OUTOKUMPU</li>
            <li>THYSSEN KRUPP (INOXUM)</li>
            <li>INDUSTEEL</li>
            <li>ACRONI</li>
        </ul><br>

        С крупнейшими азиатскими и индийскими производителями плоского проката:
        <ul>
            <li>SHANXI TAIGANG (TISCO)</li>
            <li>POSCO</li>
            <li>YUSCO</li>
            <li>LISCO</li>
            <li>BAOSTEEL</li>
            <li>ZPSS</li>
            <li>JINDAL</li>
            <li>BRG</li>
        </ul><br>

        <a title="АВ металл групп" target="_blank" href="/">АВ металл групп</a> гарантирует качество сортового проката и проволоки, так как осуществляем поставки от лучших производителей Индии:
        <ul>
            <li>VIRAJ</li>
            <li>MACROBARS</li>
            <li>PANCHMAHAL</li>
        </ul><br>

        Трубы на наших складах только итальянского производства:
        <ul>
            <li>MARCEGAGLIA</li>
            <li>PADANA</li>
            <li>ILTA INOX</li>
            <li>SIDERINOX</li>
        </ul>
    </div>

<h3 class="av-spoiler-header">Склады нержавеющего металлопроката</h3>
    <div class="av-spoiler-body">
        Наши склады расположены в следующих городах и работают с доставкой&nbsp;по всей Украине:
        <ul>
            <li><a title="Нержавеющий металл в Днепре (Днепропетровск)" target="_blank" href="/metallobaza/dnepr-obl/dnepr-filial-2/">Склад в Днепре</a></li>
            <li><a title="Нержавеющий металл в Киеве"                   target="_blank" href="/metallobaza/kiev-obl/kiev-filial-1/">Склад в Киеве</a></li>
            <li><a title="Нержавеющий металл в Одессе"                  target="_blank" href="/metallobaza/odessa-obl/odessa-metallobaza-4/">Склад в Одессе</a></li>
            <li><a title="Нержавеющий металл в Черкассах"               target="_blank" href="/metallobaza/cherkassy-obl/cherkassy-metallobaza-3/">Склад в Черкассах</a></li>
            <li><a title="Нержавеющий металл в Запорожье"               target="_blank" href="/metallobaza/zaporozhe-obl/zaporozhe-filial-1/">Склад в Запорожье</a></li>
            <li><a title="Нержавеющий металл в Харькове"                target="_blank" href="/metallobaza/kharkov-obl/kharkov-minimarket-sk-21/">Склад в Харькове</a></li>
            <li><a title="Нержавеющий металл во Львове"                 target="_blank" href="/metallobaza/lvov-obl/lvov-filial-1/">Склад во Львове</a></li>
        </ul>
    </div>
</div>