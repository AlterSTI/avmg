...
<div class="menu-items-block" id="bx-left-menu">
    <?if (!strpos($_SERVER['HTTP_USER_AGENT'], 'rv:11.0') && !strpos($_SERVER['HTTP_USER_AGENT'], 'Edge') &&
        !strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE')):?>
    <div id="warning_pointer">
				...
				if ($item["PERMISSION"] > "D")
				{
					...
					>
						...
						<a class="menu-item-link"
							href="<?=$curLink?>"
							title="<?=$item["TEXT"]?>"
							<?if ((isset($item["OPEN_IN_NEW_PAGE"]) && $item["OPEN_IN_NEW_PAGE"]) || $item["PARAMS"]['target'] == '_blank'):?>
							target="_blank"
							<?endif?>
							...
							<span
                                ...
                                "
							        menu-item-link-text
							        <?if($item["PARAMS"]["WARNING"] == 'Y'):?>menu-item-link-text-warning<?endif?>
                                "
                                data-role="item-text"
                            >
								<?=$item["TEXT"]?>
