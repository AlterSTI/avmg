...
//pre($arResult['UF_STAFF_RESPONS_ARRAY']);
/**/
			...
			switch($key)
			{
				case 'UF_DEPARTMENT':
					...
					if (is_array($arResult['USERS'][$i][$key]) && count($arResult['USERS'][$i][$key]) > 0)
					{
                        ...
                        $dataFilter = 'data-filter = "'.$arResult['USERS'][$i][$key][key($arResult['USERS'][$i][$key])].'"';
				...
				default:
				    //выборка текстового значения текстового поля

					if (substr($key, 0, 3) == 'UF_' && is_array($arResult['USER_PROP'][$key]))
					{
						$arResult['USER_PROP'][$key]['VALUE'] = $arResult['USERS'][$i][$key];

						if (in_array($arResult['USER_PROP'][$key]['USER_TYPE_ID'], array('iblock_element'))){
						    $arResult['USERS'][$i][$key] = '<span data-filter = "'.htmlspecialcharsbx($arResult['IBLOCK_ELEMENT_USER_FIELDS_VALUE'][$arResult['USERS'][$i][$key]]).'" 
                                                    data-filter-option="'.$key.'">'.
                            htmlspecialcharsbx($arResult['IBLOCK_ELEMENT_USER_FIELDS_VALUE'][$arResult['USERS'][$i][$key]]).'</span>';
						}
                        ...
                        $arResult['USERS'][$i][$key] = '<span data-filter = "'.htmlspecialcharsbx($arResult['USERS'][$i][$key]).'" 
                                                    data-filter-option="'.$key.'">'.
                                                    htmlspecialcharsbx($arResult['USERS'][$i][$key]).'</span>';
