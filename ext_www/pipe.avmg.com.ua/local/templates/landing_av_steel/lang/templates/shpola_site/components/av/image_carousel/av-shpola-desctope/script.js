...
/*--------------------------------- MAP  -----------------------------------*/
	...
	var beaches = [
		...
		['м. Генічеськ,<br>вул.Гоголя, 192', 46.1759200, 34.8034000, 11],
        ['м. Гола Пристань<br> вул.Першого Травня,300', 46.5271900, 32.5241700, 0],
        ['смт. Баришевка<br> вул.Леніна,78', 50.353889, 31.316667, 102]
	];
...
/*--------------------------------- MAP 2222 -----------------------------------*/
	...
	if(windowHeight <= 1024) {
		...
		var beaches = [
			['м. Шпола, вул. Лебединська, 4-б<br> Телефон: <a href="tel:0675660554">067 522 24 18</a>', 49.010147, 31.402838, 4],
            ['м. Генічеськ', 46.1759200, 34.8034000, 11],
            ['м. Гола Пристань', 46.5271900, 32.5241700, 0],
            ['смт. Баришевка', 50.353889, 31.316667, 102]
		];
	...
	/*--------------------------------- ПОЯВЛЕНИЕ КАРТЫ ПРИ КЛИКЕ  -----------------------------------*/
    function googleMapMobileUp() {
        var windowHeight = window.innerHeight;
        if(windowHeight <= 1024) {
            var mapDiv = document.getElementById('map-open');
            var map2;

            function initialize() {
                var map2 = new google.maps.Map(document.getElementById("map_canvas_big"), {
                    center: {
                        lat: 49.010147,
                        lng: 31.402838
                    },
            ...
            function setMarkers(map2) {
                if ($('#map_canvas_big').is('[markers-seted]')) return;
                var image = {
                    url: '/upload/av_site/landings/av-steel/map-marker-new.png',
                    // This marker is 20 pixels wide by 32 pixels high.
                    size: new google.maps.Size(40, 60),
                ...
                $('#map_canvas_big').attr('markers-seted', true);
            }
            google.maps.event.addDomListener(window, 'load', initialize);
    ...
    $('#map-open').click(function() {
        $('#map_canvas_big').addClass("map-enable");
        $('.back').addClass("map-vis-back");
        googleMapMobileUp();
    });

    $('.back').click(function() {
        $('#map_canvas_big').removeClass("map-enable");
        $('.back').removeClass("map-vis-back");
	});
...
function setHeight() {
	...
	$('.sliderText').each(function(){
		...
		if(windowHeight >= 600) {
			...
			$(this).css("margin-top", ((windowHeight / 2 - 30) - $imgTextHeight / 2) + "px");
            $('.mobileWrap .logoTextWrap').css("margin-top", (windowHeight /  windowHeight - 1) + "px");
		}
