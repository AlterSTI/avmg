...
class CIntranetStructureListComponent extends CBitrixComponent
{
	...
	public $bExcel = null;
	/*
	 * Список базовых полей, по которым не проводить фильтрацию
	 * */
	protected $arUserFieldNamesNotFilter = array(
	    'PERSONAL_PHOTO', 'FULL_NAME', 'DATE_REGISTER','PERSONAL_STATE',
        'PERSONAL_ZIP','PERSONAL_COUNTRY','PERSONAL_NOTES', 'ADMIN_NOTES','XML_ID');
	...
	protected function fillFilter()
	{
        ...
        //если поле пользовательское
        unset($this->arFilter['ACTIVE']);
		//$arAjaxFilter - массив результирующего фильтра
        $arAjaxFilter = array("LOGIC" => "OR", 'ACTIVE' => 'Y');
        foreach ($this->arParams['USER_PROPERTY_TABLE'] as $key => $ufFilter) {
           /* if ($this->arParams['DATA_FILTER_OPTION'] == $ufFilter) echo $this->arParams['DATA_FILTER_OPTION'].'<br>';
           заготовка под более сложный фильтр
           /**/
            //определяем тип пользовательского поля
           if (substr($ufFilter, 0, 3) == 'UF_') {
                $rsEnum = CUserTypeEntity::GetList(array(), array("FIELD_NAME" => $ufFilter));
                while ($res = $rsEnum->Fetch()) {
                    //$res['SETTINGS']['IBLOCK_ID'] - инфоблок, к которому привязано пользовательское поле

                    switch ($res['USER_TYPE_ID']) {
                        case 'iblock_section':
                            $resBlock = CIBlockSection::GetList(
                                array("SORT" => "ASC"),
                                array('IBLOCK_ID' => $res['SETTINGS']['IBLOCK_ID'], 'NAME' => "%".$this->arParams['NEED_SEARCH_STRING']."%"),
                                false,
                                array('ID', 'NAME'),
                                false
                            );
                            //получаем массив типа UF_DEPARTMENT
                            while ($resultBlock = $resBlock->Fetch()) {
                                $arRes[$resultBlock['NAME']] = $resultBlock['ID'];
                            }
                            if (count($arRes)>0) {
                                $arAjaxFilter[]=array($res['FIELD_NAME'] => $arRes);
                            } else
                        break;
                        case 'iblock_element':
                            ...
                            //получаем массив типа UF_STAFF_RESPONS
                            while ($resultElement = $resElement->Fetch()) {
                                $arResEl[$resultElement['NAME']] = $resultElement['ID'];
                            }
                            if (count($arResEl)>0) {
                                $arAjaxFilter[]=array($res['FIELD_NAME'] => $arResEl);
                            }
                        break;

                        default://case 'string':
                            if ($this->arParams['NEED_SEARCH_STRING'] != ''){
                                $arAjaxFilter[]=array($res['FIELD_NAME'] => $this->arParams['NEED_SEARCH_STRING']);

                            }
                        break;
                        }
                    }
            } else if ($ufFilter == 'FULL_NAME'){
               $this->arFilter['NAME'] = $this->arParams['NEED_SEARCH_STRING'];
               $this->arFilter['LAST_NAME'] = $this->arParams['NEED_SEARCH_STRING'];
               $this->arFilter['SECOND_NAME'] = $this->arParams['NEED_SEARCH_STRING'];
            } else {
                //добавляем как строки остальные стандартные поля
               $this->arFilter[$ufFilter] = $this->arParams['NEED_SEARCH_STRING'];
            }
        }
        //добавляем массив фильтра к общему $this->arFilter
        $this->arFilter[] = $arAjaxFilter;
       /* echo '</pre>';/**/
        if (!$this->arParams['NEED_SEARCH_STRING'] || $this->arParams['NEED_SEARCH_STRING'] == "%%"){
            $this->arFilter = array(
                                    "ACTIVE" => "Y",
                                    "!UF_DEPARTMENT" =>'',
                                    "!EXTERNAL_AUTH_ID" => array('replica', 'email', 'imconnector', 'saleanonymous')
                                    );
        }
        //echo '<pre>';
        //print_r($this->arFilter);
        //print_r($arAjaxFilter);
        ...
        if (IsModuleInstalled('imconnector'))
        {
f            $arExternalAuthId[] = 'imconnector';
        }
...
if ($isEnoughFiltered) $t=1; else $t=0;
        echo '<pre>';
        print_r($t);
        echo '</pre>';
	...
	protected function getCacheIdWithDepartment($reload = false)
	{
        ...
        $resCashArray = array('filter'=> $this->arFilter,
                              'iNumPage' => $this->arParams['I_NUM_PAGE'] != '' ? $this->arParams['I_NUM_PAGE']:1,
                              'usersPerPage' => $this->arParams['USERS_PER_PAGE'],
                              'user' => $GLOBALS['USER']->GetUserGroupString($GLOBALS['USER']->GetID()),
                              'active' => $this->arParams['ACTIVE_USER']
        );
        $cntStartCacheId = md5(serialize($resCashArray));

		if($cntStartCacheId && !$reload)
		{
			//return array($cntStartCacheId, $cacheCount);
            return $cntStartCacheId;
		}
	...
	public function executeComponent()
	{
        ...
        $cntStartCacheId = $this->getCacheIdWithDepartment();
		...
		$this->arParams['bCache'] =
            $cntStartCacheId // we cache only unfiltered list
			&& !$this->bExcel
		...
		if(!$bFromCache)
		{
			...
			else
			{
				...
				{
					$arListParams['NAV_PARAMS'] = array('nPageSize' => $this->arParams['USERS_PER_PAGE'], 'iNumPage'=>$this->arParams['I_NUM_PAGE'], 'bShowAll' => false);
				}
                /*echo 'Выполнен запрос <br />';
                echo '<pre>';
				print_r($this->arFilter);
                echo '</pre>';/**/
			...
			while ($arUser = $dbUsers->Fetch()) {
                ...
                } else if ($this->arParams['ACTIVE_USER'] === 'false'){
                    $this->arResult['USERS'][$arUser['ID']] = $arUser;
                    $strUserIDs .= ($strUserIDs === '' ? '' : '|') . $arUser['ID'];
                }
            }
			...
			unset($arUser, $key);
			$this->arResult["USERS_NAV"] = $bNav ? $dbUsers->GetPageNavStringEx(
			            $navComponentObject=null, $this->arParams["NAV_TITLE"],
                        'av_corp_ajax', false, null, array("NEED_SEARCH_STRING" => $this->arParams['NEED_SEARCH_STRING'])) : '';
