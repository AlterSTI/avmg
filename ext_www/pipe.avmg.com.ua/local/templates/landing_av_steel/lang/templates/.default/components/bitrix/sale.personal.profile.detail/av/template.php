...
if(strlen($arResult["ID"])>0)
{
	...
	<form method="post"  class="col-md-12 sale-profile-detail-form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
		...
		<div class="form-group">
			<label class=""><?=Loc::getMessage('SALE_PERS_TYPE')?></label>

				<?=$arResult["PERSON_TYPE"]["NAME"]?>

		</div>
		<div class="form-group">
			<label class="" for="sale-personal-profile-detail-name">
			...
			if (!empty($block["PROPS"]))
			{
				...
				foreach($block["PROPS"] as $key => $property)
				{
					...
					<div class="form-group sale-personal-profile-detail-property-<?=strtolower($property["TYPE"])?>">
						...
						<label class="<?=$alignTop?>" for="sppd-property-<?=$key2?>">
							<?= $property["NAME"]?>:
							<?
		...
		?>
		<div class=" col-sm-9 sale-personal-profile-btn-block">
			<input type="submit" class="btn btn-themes btn-default btn-md apply" name="save" value="<?echo GetMessage("SALE_SAVE") ?>">
			&nbsp;
			<input type="submit" class="btn btn-themes btn-default btn-md save"  name="apply" value="<?=GetMessage("SALE_APPLY")?>">
			&nbsp;
